package com.cyh.flinkProject.isSpider.ruleCompute

import java.util
import java.util.Date

import com.cyh.flinkProject.isSpider.common.bean.{FlowCollocation, FlowScoreResult, ProcessedData}
...
object RuleUtils {
    ...
    for (flow <- flowList) {
      ...
      for (rule <- ruleList) {
        ...
        if (streamRuleValue > mysqlRuleValue) {
          ...
          println(s"命中的规则名称：${rule.ruleName},计算出的结果值：${streamRuleValue} 数据库配置的阈值：${mysqlRuleValue} 给定的分值：${ruleScore}")
        } else {
          //没命中，打分设置为0
      ...
      flowScores.append(FlowScoreResult(flow.flowId, flowScore, flow.flowLimitScore, flowScore > flow.flowLimitScore, hitRules.toList,hitTime))

      println(s"flowId:${flow.flowId}  flowLimitScore:${flow.flowLimitScore} flowScore:$flowScore")
    }
  ...
  def computeScore(scores: Array[Double],xa: Array[Double]): Double = {
    ...
    //todo: 系数2：系数2的权重是60，指的是最高score以6为分界，最高score大于6，就给满权重60，不足6，就给对应的maxInXa*10
    val factor2 = if (1 < (1.0 / 6.0) * maxInXa) {
      60
    } else {
      (1.0 / 6.0) * maxInXa * 60
    }
    //todo: 系数3：打开的规则总分占总规则总分的百分比，并且系数3的权重是40
