package com.cyh.flinkProject.isSpider.dataProcess.mysqlSource

import java.sql.{Connection, PreparedStatement, ResultSet}
import java.util

import com.cyh.flinkProject.isSpider.common.bean.{FlowCollocation, RuleCollocation}
import com.cyh.flinkProject.isSpider.common.util.database.C3p0Util
import com.cyh.flinkProject.isSpider.common.util.jedis.JedisConnectionUtil
...
class MysqlProcessRuleSource extends RichSourceFunction[util.HashMap[String,Any]]{
  ...
  def queryflowRuleToMap(rs: ResultSet, map: util.HashMap[String, Any]): util.HashMap[String, Any] ={
    ...
    while (rs.next()) {
      ...
      //获取流程列表，封装数据
      flowRuleArray += new FlowCollocation(flowId, flowName, createRuleList(flowId), flowLimitScore)
